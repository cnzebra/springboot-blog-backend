package com.cnlm.blog.utils;

import com.cnlm.blog.constant.WebConst;
import com.cnlm.blog.controller.admin.AttachController;
...
public class TaleUtils {
     ...
     * 匹配邮箱正则
     */
    private static final Pattern VALID_EMAIL_ADDRESS_REGEX = Pattern.compile("^[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2," +
    ...
    private static Pattern scriptPrefixPattern = Pattern.compile("<script(.*?)>", Pattern.CASE_INSENSITIVE | Pattern
            .MULTILINE | Pattern
            .DOTALL);

    private static Pattern scriptSurfixPattern = Pattern.compile("</script>", Pattern.CASE_INSENSITIVE);

    private static Pattern evalPattern = Pattern.compile("eval\\((.*?)\\)", Pattern.CASE_INSENSITIVE | Pattern
            .MULTILINE | Pattern
            .DOTALL);

    private static Pattern expressionPattern = Pattern.compile("expression\\((.*?)\\)", Pattern.CASE_INSENSITIVE |
            Pattern.MULTILINE |
            Pattern.DOTALL);
    ...
    private static Pattern onloadPattern = Pattern.compile("onload(.*?)=", Pattern.CASE_INSENSITIVE | Pattern
            .MULTILINE | Pattern
            .DOTALL);
...
//            默认是classPath路径
    ...
    public static String filterXSS(String value) {
        ...
        if (value != null) {
            ...
            cleanValue = srcPattern.matcher(cleanValue).replaceAll("");

            // Remove any lonesome </script> tag
            cleanValue = scriptSurfixPattern.matcher(cleanValue).replaceAll("");

            // Remove any lonesome <script ...> tag

            cleanValue = scriptPrefixPattern.matcher(cleanValue).replaceAll("");

            // Avoid eval(...) expressions

            cleanValue = evalPattern.matcher(cleanValue).replaceAll("");

            // Avoid expression(...) expressions

            cleanValue = expressionPattern.matcher(cleanValue).replaceAll("");

            // Avoid javascript:... expressions
            cleanValue = javascriptPattern.matcher(cleanValue).replaceAll("");

            // Avoid vbscript:... expressions
            cleanValue = vbscriptPattern.matcher(cleanValue).replaceAll("");

            // Avoid onload= expressions

            cleanValue = onloadPattern.matcher(cleanValue).replaceAll("");
        }
        return cleanValue;
    }
