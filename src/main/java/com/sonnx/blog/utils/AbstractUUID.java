...
public abstract class AbstractUUID {
    ...
    private static final char[] UU64 = "-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz"
            .toCharArray();
    ...
    /**
     * @return 64进制表示的紧凑格式的 AbstractUUID
     */
    ...
    /**
     * 返回一个 AbstractUUID ，并用 64 进制转换成紧凑形式的字符串，内容为 [\\-0-9a-zA-Z_]
     * <p>
     * 比如一个类似下面的 AbstractUUID:
     * <p>
     * <pre>
     * a6c5c51c-689c-4525-9bcd-c14c1e107c80
     * 一共 128 位，分做L64 和 R64，分为为两个 64位数（两个 long）
     *    > L = uu.getLeastSignificantBits();
     *    > AbstractUUID = uu.getMostSignificantBits();
    ...
    /**
     * 从一个 uu64 恢复回一个 AbstractUUID 对象
     *
     * @param uu64 64进制表示的 AbstractUUID, 内容为 [\\-0-9a-zA-Z_]
     * @return AbstractUUID 对象
     */
    ...
    /**
     * 将紧凑格式的 UU16 字符串变成标准 AbstractUUID 格式的字符串
     *
     * @param uu16
     * @return 标准 AbstractUUID 字符串
     */
     ...
     *
     * @param uu64
     *            uu64 64进制表示的 AbstractUUID, 内容为 [\\-0-9a-zA-Z_]
     * @return 16进制表示的紧凑格式的 AbstractUUID
     */
